...
function conflictReasons(candidate, other, prefix) {
  ...
  if (semantic.length) {
    reasons.push({ kind: prefix + '-resource-overlap', work: [other.id], overlaps: semantic });
  }
  return reasons;
}
...
export function buildWorkerQueue(rows) {
  ...
  for (const candidate of candidates) {
    ...
    for (const dependency of candidate.dependsOn ?? []) {
      ...
      else if (FAILED.has(state.status)) reasons.push({ kind: 'failed-dependency', work: dependency, status: state.status });
      else if (state.status !== 'accepted') reasons.push({ kind: 'waiting-dependency', work: dependency, status: state.status });
    }

    for (const reserved of reservations) {
      if (reserved.id !== candidate.id) reasons.push(...conflictReasons(candidate, reserved, 'reserved'));
    }
